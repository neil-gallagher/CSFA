...
for l = 1:size(scoresAll,1)
    ...
    vals2 = sum(bsxfun(@times,UKUlStorePlus, ...
        permute(scoresAll(:,w),[2,1])),2); % get nonzero values
    UKUPlus = sparse(rows,cols,vals2,Nc*Ns,Nc*Ns); % put into sparse matrix
    UKUPlus = UKUPlus + 1/gam * speye(Nc*Ns); % add white noise
    UKUPlus = 2*UKUPlus;
   ...
   [~,UKUlPlus] = Kl.UKU(s,opts); % get factor l Gram matrix
    ...
    UKUMinus = UKUMinus + 1/gam * speye(Nc*Ns); % add white noise
    UKUMinus = 2*UKUMinus;
